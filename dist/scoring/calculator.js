...
function getGrade(score) {
    if (score >= 90)
        return { grade: 'excellent', label: GRADE_LABELS.excellent };
    if (score >= 70)
        return { grade: 'good', label: GRADE_LABELS.good };
    if (score >= 50)
        return { grade: 'fair', label: GRADE_LABELS.fair };
...
function calculateScore(results) {
    ...
    for (const [category, items] of grouped) {
        const weight = exports.CATEGORY_WEIGHTS[category] ?? 1.0;
        // unknown 不计入分母
